...
// MAGIC While we might be generating the same shape of data for every device, every device is different. In this exercise, we'll be adding some noise using the formula `randn(seed)*10` in a new column called 'co2_ppm_noise'.
// MAGIC 
// MAGIC ### Resources
...
def addNoise(seed: Long = Utils.random.nextLong): DataFrame => DataFrame =
  df =>
    // Fill in ???. 
    df.withColumn("co2_ppm_noise", col("co2_ppm_3_8") + randn(seed)*10)
...
def generateSingleSample(initialDf: DataFrame, deviceNumber: Int, seed: Option[Long] = None): DataFrame = {
  initialDf
    ...
    .transform(scaleDistBy(5))
    .transform(scaleBy38())
//     .transform(if (seed == None) addNoise() else addNoise(seed))
    .transform(
      seed match {
...
def generateDataForAllDevices(initialDf: DataFrame, deviceNumbers: Seq[Int], seed: Option[Long] = None): Seq[DataFrame] = {
  deviceNumbers.map(x => generateSingleSample(initialDf, x, seed)).toSeq
}
...
/// YOUR CODE HERE
// Replace ???
val co2PpmWindowSpec = Window.partitionBy("device").orderBy("interval").rowsBetween(-2, 2)
...
def co2PpmMovingAverage(winSpec: WindowSpec): DataFrame => DataFrame =
  df =>df
    .select(
      ...
      col("interval"),
      col("co2_ppm"),
      avg($"co2_ppm").over(winSpec) as "avg_window_co2_ppm")
...
def co2PpmSlopeMovingAverage(winSpec: WindowSpec): DataFrame => DataFrame =
  df =>df
    .withColumn("lag", lag("co2_ppm", 1).over(Window.partitionBy("device").orderBy("interval")))
    .withColumn("slope", col("co2_ppm")-col("lag"))
    .select(col("interval"), col("device"), col("co2_ppm"), col("avg_window_co2_ppm"), avg(col("slope")).over(winSpec) as "avg_slope")
...
def thresholdCrossed(): DataFrame => DataFrame =
  df =>df
    .filter(col("avg_slope") > 0 && col("avg_window_co2_ppm") > 1000)
    .withColumn("row_index", row_number.over(Window.partitionBy("device").orderBy("interval")))
    .where(col("row_index") === 1)
    .drop("row_index")
